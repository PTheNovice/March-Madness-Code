...
def rpi_stat_comp(df):
    # Multiplied by the arbitrary Average of other teams
    df['A_RPI'] = (df['A_WinRatio'] * .25) + (df['B_WinRatio'] * .5) + (((df['A_WinRatio'] + df['B_WinRatio']) / 2) * .25)
    df['B_RPI'] = (df['B_WinRatio'] * .25) + (df['A_WinRatio'] * .5) + (((df['A_WinRatio'] + df['B_WinRatio']) / 2) * .25)
    return df
...
def test_train_df(s_hist_agg):
    ...
    loss = train_df[['Season', "BTeamID","ATeamID",'B_Matches_Played', 'B_Score', 'B_OppScore', 'B_NumOT',
       ...
       'A_DRR', 'A_OppDRR', 'A_AstRatio', 'A_OppAstRatio', 'A_FGM2','A_OppFGM2','A_FGR','A_OppFGR','A_ORCHANCE','A_OppORCHANCE',
        'A_PFDIFF','A_OppPFDIFF','A_FGADIFF','A_OppFGADIFF','A_FGA2','A_OppFGA2','A_FG2PCT','A_OppFG2PCT','A_ScoreDiff','A_TODIFF','A_STLDIFF','A_BLKDIFF', 'A_Pace',
    ...
    # train_df_all = train_df[(train_df["Season"] < 2023)]
    # train_df_rec = train_df[(train_df["Season"] < 2023)]
    ...
    f_df[['A_Matches_Played', 'A_Score', 'A_OppScore',
       'A_NumOT', 'A_FGM', 'A_FGA', 'A_FGM3', 'A_FGA3', 'A_FTM', 'A_FTA',
       'A_OR', 'A_DR', 'A_Ast', 'A_TO', 'A_Stl', 'A_Blk', 'A_PF', 'A_OppFGM',
       'A_OppFGA', 'A_OppFGM3', 'A_OppFGA3', 'A_OppFTM', 'A_OppFTA', 'A_OppOR',
       'A_OppDR', 'A_OppAst', 'A_OppTO', 'A_OppStl', 'A_OppBlk', 'A_OppPF',
       ...
       'A_OppPPSA', 'A_FGperc', 'A_OppFGperc', 'A_3Fper', 'A_Opp3Fper',
       'A_ToRatio', 'A_OppToRatio', 'A_Matches_Won', 'A_WinRatio',
       'A_AvgScore', 'A_AvgOppScore', 'A_LossRatio', 'B_Matches_Played', 'B_Score', 'B_OppScore', 'B_NumOT',
       'B_FGM', 'B_FGA', 'B_FGM3', 'B_FGA3', 'B_FTM', 'B_FTA', 'B_OR', 'B_DR',
       'B_Ast', 'B_TO', 'B_Stl', 'B_Blk', 'B_PF', 'B_OppFGM', 'B_OppFGA',
       'B_OppFGM3', 'B_OppFGA3', 'B_OppFTM', 'B_OppFTA', 'B_OppOR', 'B_OppDR',
       'B_OppAst', 'B_OppTO', 'B_OppStl', 'B_OppBlk', 'B_OppPF', 'B_POSS',
       ...
       'B_FGperc', 'B_OppFGperc', 'B_3Fper', 'B_Opp3Fper', 'B_ToRatio',
       'B_OppToRatio', 'B_Matches_Won', 'B_WinRatio', 'B_AvgScore',
       'B_AvgOppScore', 'B_LossRatio']] = scale.fit_transform(
        train_df[['A_Matches_Played', 'A_Score', 'A_OppScore',
       'A_NumOT', 'A_FGM', 'A_FGA', 'A_FGM3', 'A_FGA3', 'A_FTM', 'A_FTA',
       'A_OR', 'A_DR', 'A_Ast', 'A_TO', 'A_Stl', 'A_Blk', 'A_PF', 'A_OppFGM',
       'A_OppFGA', 'A_OppFGM3', 'A_OppFGA3', 'A_OppFTM', 'A_OppFTA', 'A_OppOR',
       'A_OppDR', 'A_OppAst', 'A_OppTO', 'A_OppStl', 'A_OppBlk', 'A_OppPF',
    ...
    # features = f_df.corr()['AWin'][f_df.corr()["AWin"] >= 0.95].index.tolist()
    # print("These are the features", features)
    #
    # select_feats_1 = f_df.corr()['AWin'][f_df.corr()["AWin"].abs() >= 0.2].index.tolist()
    # select_feats = f_df.corr()['AWin'][f_df.corr()["AWin"].abs() >= 0.2].index
    # print("These are the features", select_feats_1)
    #
    # pca = PCA(n_components=5)
    # new_features = pca.fit_transform(f_df[select_feats])
    # print("These are the new features", new_features)

    f_df = f_df.sample(frac=1).reset_index(drop=True)

    test_df = f_df[f_df["Season"] >= 2023]
    train_df_rec = f_df[(f_df["Season"] < 2023)]  #All Games
    train_df_all = f_df[(f_df["Season"] < 2023) & (f_df["Season"] >= 2019)]  #Recent Games

    final_train_df_rec = train_df_rec.drop(columns=['Season', 'ATeamID', 'BTeamID'])
    final_train_df_all = train_df_all.drop(columns=['Season', 'ATeamID', 'BTeamID'])
    final_test_df = test_df.drop(columns=['Season', 'ATeamID', 'BTeamID'])

    # final_test_df.to_csv('data/finaltest.csv')
    # final_train_df.to_csv('data/finaltrain.csv')

    X_train_rec = final_train_df_rec.iloc[:, :-1]
    X_train_all = final_train_df_all.iloc[:, :-1]
    X_test = pd.DataFrame(scale.fit_transform(final_test_df)).iloc[:, :-1]
    y_train_rec = final_train_df_rec.iloc[:, -1]
    y_train_all = final_train_df_all.iloc[:, -1]
    y_test = final_test_df.iloc[:, -1]

    return X_train_rec, X_test, y_train_rec, y_test, X_train_all, y_train_all
...
def cinderella_weight(teamA_stats, teamB_stats, seedA, seedB, teamA_id, teamB_id):
    ...
    #     seedB = seedA
    #     seedA = tempSeed
    #     tempStats = teamB_stats
    #     teamB_stats = teamA_stats
    #     teamA_stats = tempStats

    # if (seedB - seedA) >= 5 and np.int32(teamB_stats['B_WinRatio']) > np.int32(teamA_stats['A_WinRatio']):
    if (seedB - seedA) >= 5:
        weight += 1
    if 'B_WinRatio' in teamB_stats and 'A_WinRatio' in teamA_stats:
        if np.int32(teamB_stats['B_WinRatio']) > np.int32(teamA_stats['A_WinRatio']):
            weight += .1
    if 'B_NET_RAT' in teamB_stats and 'A_NET_RAT' in teamA_stats:
        if np.int32(teamB_stats['B_NET_RAT']) > np.int32(teamA_stats['A_NET_RAT']):
            weight += .20
    if 'B_ToRatio' in teamB_stats and 'A_ToRatio' in teamA_stats:
        if np.int32(teamB_stats['B_ToRatio']) > np.int32(teamA_stats['A_ToRatio']):
            weight += .1
    ...
    if teamA_id in kpom['TeamID'].values and teamB_id in kpom['TeamID'].values:
        if kpom.loc[kpom['TeamID'] == teamA_id, 'luck'].values[0] > kpom.loc[kpom['TeamID'] == teamB_id, 'luck'].values[0]:
            weight += .25
...
def svm_model(X_train, X_test, y_train, y_test):
    # svm = SVR(kernel='poly', degree=3, gamma='auto')
    ...
    # fig, ax = plt.subplots(1, 1, figsize=(12, 8))
    # sns.heatmap(svm_cm, annot=True, ax=ax, fmt='g', cmap='Blues')
...
def who_won(teamA_id, seed1, teamB_id, seed2, s_hist_agg, lr, rf, svm):
    ...
    teamA_stats = teamA_stats[
        ['A_Matches_Played', 'A_Score', 'A_OppScore',
       'A_NumOT', 'A_FGM', 'A_FGA', 'A_FGM3', 'A_FGA3', 'A_FTM', 'A_FTA',
       'A_OR', 'A_DR', 'A_Ast', 'A_TO', 'A_Stl', 'A_Blk', 'A_PF', 'A_OppFGM',
       'A_OppFGA', 'A_OppFGM3', 'A_OppFGA3', 'A_OppFTM', 'A_OppFTA', 'A_OppOR',
       'A_OppDR', 'A_OppAst', 'A_OppTO', 'A_OppStl', 'A_OppBlk', 'A_OppPF',
    ...
    teamB_stats = teamB_stats[['B_Matches_Played', 'B_Score', 'B_OppScore', 'B_NumOT',
       'B_FGM', 'B_FGA', 'B_FGM3', 'B_FGA3', 'B_FTM', 'B_FTA', 'B_OR', 'B_DR',
       'B_Ast', 'B_TO', 'B_Stl', 'B_Blk', 'B_PF', 'B_OppFGM', 'B_OppFGA',
       'B_OppFGM3', 'B_OppFGA3', 'B_OppFTM', 'B_OppFTA', 'B_OppOR', 'B_OppDR',
       'B_OppAst', 'B_OppTO', 'B_OppStl', 'B_OppBlk', 'B_OppPF', 'B_POSS',
    ...
    new_game_features = pd.concat([teamA_stats.reset_index(drop=True), teamB_stats.reset_index(drop=True)], axis=1)
    new_game_features = rpi_stat_comp(new_game_features)
    ...
    c_weight, seed = cinderella_weight(teamA_stats, teamB_stats, seed1, seed2, teamA_id, teamB_id)
    print("The cinderella weight: ", c_weight)

    win_prob = (((np.int32(rf_prob[0][1]) + np.int32(lr_prob[0][1])) / 2) -
                (((np.int32(rf_prob[0][0]) * c_weight) + np.int32(lr_prob[0][0])) / 2))

    y = 0

    if win_prob > 0:
        print("Team A is the winner!!!!!!", win_prob)
        winner = teama_name
        y = 1
    else:
        winner = teamb_name
    ...
    return lr_prob, rf_prob, svm_prob, winner_dict, y
...
def fit_models():
    ...
    X_train, X_test, y_train, y_test, X_train_all, y_train_all = test_train_df(season_data)

    lr = log_model(X_train_all, X_test, y_train_all, y_test)
...
def test_project():
    ...
    lr_prob, rf_prob, svm_prob, game_stats, y = who_won(teamA_id=1163, seed1=seed1, teamB_id=1391, seed2=seed2, s_hist_agg=season_data, lr=lr, rf=rf, svm=svm)
    ...
    # Sample output of the final matrix for 2024
    ...
    #
    #     # TeamA = games.iloc[i]['TeamA']
    #     # TeamB = games.iloc[i]['TeamB']
    #     TeamA = matchup['TeamA']
    #     TeamB = matchup['TeamB']
    #
    ...
    #     lr, rf, svm, season_data = fit_models()
    #
    #     lr_prob, rf_prob, svm_prob, game_stats, y = who_won(teamA_id=TeamA, seed1=seed1, teamB_id=TeamB, seed2=seed2,
    #                                                      s_hist_agg=season_data, lr=lr, rf=rf, svm=svm)
    #     y_pred.append(y)
    #
    #     print("Probabilities of Team A vs Team B", lr_prob, rf_prob, svm_prob)
    #     print("The Game Stats!", game_stats)
